...
class UnifiedHistoryServiceImpl implements UnifiedHistoryService {
  ...
  Future<void> addShareHistory(ShareContent shareContent) async {
    try {
      ...
      // 创建 SharedContent 对象用于持久化，确保使用正确的标题
      final sharedContent = SharedContent(
        id: shareContent.id,
        text: shareContent.title, // 使用 ShareContent 的 title 作为 text
        images: shareContent.originalContent?.images ?? const [],
  ...
  /// 获取分享历史记录
  Future<List<ShareContent>> _getShareHistories() async {
    try {
      final shareHistories = await _localStorageService.getShareHistory();
      return shareHistories.map((history) => ShareContent.fromShareHistory(history)).toList();
    } catch (e) {
  ...
  /// 获取录音历史记录
  Future<List<AudioRecord>> _getAudioHistories() async {
    try {
      if (_appDocumentsPath == null) return [];
      ...
      final audioHistories = <AudioRecord>[];
      ...
      for (final dir in subDirs) {
        try {
          ...
          if (await metadataFile.exists()) {
            ...
            final audioRecord = AudioRecord.fromJson(metadataData);
            audioHistories.add(audioRecord);
          }
        } catch (e) {
