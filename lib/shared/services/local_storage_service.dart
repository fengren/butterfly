...
abstract class LocalStorageService {
  ...
  Future<void> saveShareHistory(ShareHistory history);
}
...
class LocalStorageServiceImpl implements LocalStorageService {
  ...
  @override
  Future<void> saveShareHistory(ShareHistory history) async {
    try {
      // 创建目录名（基于时间戳）
      final timestamp = history.createdAt;
      final dirName = '${timestamp.year.toString().padLeft(4, '0')}${timestamp.month.toString().padLeft(2, '0')}${timestamp.day.toString().padLeft(2, '0')}_${timestamp.hour.toString().padLeft(2, '0')}${timestamp.minute.toString().padLeft(2, '0')}${timestamp.second.toString().padLeft(2, '0')}';
      
      final localDirectory = path.join(_appDocumentsPath!, 'shared_content', dirName);
      await createDirectory(localDirectory);
      
      // 保存历史记录信息到content.json文件
      final contentFile = File(path.join(localDirectory, 'content.json'));
      final contentData = {
        'id': history.id,
        'text': history.title,
        'images': <Map<String, dynamic>>[],
        'receivedAt': history.createdAt.millisecondsSinceEpoch,
        'sourceApp': history.sourceApp,
        'localDirectory': dirName,
      };
      await contentFile.writeAsString(json.encode(contentData));
      
      print('$_tag: Saved share history to $localDirectory');
    } catch (e) {
      print('$_tag: Failed to save share history: $e');
      rethrow;
    }
  }
